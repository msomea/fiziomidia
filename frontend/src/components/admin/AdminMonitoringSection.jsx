...
import { getAdminActivityLogs, getAdminStats } from "../../api/admin";
...
export default function AdminMonitoringSection() {
  ...
  const loadLogs = async () => {
    try {
      ...
      const data = await getAdminActivityLogs(params);
      setLogs(data.logs);
      ...
      <div className="space-y-4">
        ...
        {showStats && stats && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 p-4 bg-gray-50 rounded">
            {/* Action Stats */}
            <div>
              <h3 className="font-semibold text-caribbean mb-2">{t('action_breakdown')}</h3>
              <div className="space-y-1">
                {stats.actionStats.slice(0, 5).map((stat) => (
                  <div key={stat._id} className="flex justify-between text-sm">
                    <span className={getActionColor(stat._id)}>
            ...
            <div>
              ...
              <div className="space-y-1">
                {stats.adminStats.slice(0, 5).map((stat) => (
                  <div key={stat._id} className="flex justify-between text-sm">
                    <span>{stat.adminName}</span>
                    <span className="font-medium">{stat.count}</span>
                  </div>
                ))}
              </div>
            ...
            {/* Recent Activity */}
            <div>
              <h3 className="font-semibold text-caribbean mb-2">{t('recent_activity')}</h3>
              <div className="text-sm text-gray-600">
                {t('last_7_days')}: {stats.dailyStats.reduce((sum, day) => sum + day.count, 0)} {t('actions')}
              </div>
            </div>
          </div>
        )}
